...
import { getTotalFriendUnreadCount, getTotalGroupUnreadCount, getCachedGroupConversations, getIncomingRequestsCount } from "@/api";
...
export function isNotificationSoundEnabled(): boolean {
  const value = readHubPath<boolean>(STORAGE_PATH, true); // Default: true
  return value === true;
}
...
export function setNotificationSoundEnabled(enabled: boolean): void {
  writeHubPath(STORAGE_PATH, enabled);
}
...
export function getMutedGroupIds(): number[] {
  return readHubPath<number[]>(MUTED_GROUPS_PATH, []) ?? [];
}
...
export function toggleGroupMute(groupId: number): boolean {
  ...
  if (index === -1) {
    writeHubPath(MUTED_GROUPS_PATH, [...current, groupId]);
    return true;
  } else {
    writeHubPath(MUTED_GROUPS_PATH, current.filter((id) => id !== groupId));
    return false;
  }
}
