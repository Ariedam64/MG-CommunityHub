...
import { PetsService } from "@/game/petAbilities";
import { readAriesPath } from "@/storage/storage";
import type { PetTeam } from "@/game/petAbilities";
import { getAbilityChipColors } from "@/game/petAbilities";
    ...
    if (SIMPLE_ITEM_TYPES.has(token.type)) {
      ...
      if (token.meta) {
        try {
          ...
          const pObj = p as { mutations?: unknown } | null;
          if (typeof p === "object" && pObj !== null && Array.isArray(pObj.mutations)) {
            muts = pObj.mutations as string[];
