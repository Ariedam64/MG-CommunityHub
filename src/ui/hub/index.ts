...
type TabId = "messages" | "community" | "room" | "groups" | "leaderboard" | "search" | "myProfile";
...
class CommunityHub {
  ...
  private updateAllBadges(): void {
    ...
    // Check and play notification sound (exclude muted groups)
    const unmutedGroupUnread = getUnmutedGroupUnreadCount();
    checkAndPlayNotificationSound(friendUnread, unmutedGroupUnread, requestsUnread);
  ...
  private setOpen(open: boolean): void {
    // Guard: destroyed instances must never update DOM or dispatch events.
    if (this.destroyed) return;
    ...
    if (open) {
      ...
      // Wrap in try-catch so a throwing event handler cannot escape setOpen
      // and leave panelOpen stuck at true with the panel visually hidden.
      this._isSelfDispatching = true;
      try {
        window.dispatchEvent(new CustomEvent(CH_EVENTS.OPEN));
      } catch (e) {
        console.error("[CommunityHub] CH_EVENTS.OPEN handler threw:", e);
      } finally {
      ...
      try {
        ...
        console.error("[CommunityHub] CH_EVENTS.CLOSE handler threw:", e);
      } finally {
        this._isSelfDispatching = false;
      }
    }
