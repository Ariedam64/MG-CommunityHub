...
const STORE_OWNER = "mg-community-hub";
...
export async function ensureStore(): Promise<JotaiStore> {
  ...
  try {
    // Route through the cross-mod bridge: if Arie's Mod (or any other mod
    // speaking the protocol) already captured the store on this page, reuse
    // it instead of running a second capture.
